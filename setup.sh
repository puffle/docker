...
git clone -b $CLIENT_BRANCH --depth 1 https://github.com/puffle/client.git client
git clone -b $SERVER_BRANCH --depth 1 https://github.com/puffle/server.git server
...
# Prisma
echo "TASK: Pushing database schema..."
cd $BASE_DIR/server
npx prisma db push
...
tar -xzvf /tmp/dockerize.tar.gz -C .data/
